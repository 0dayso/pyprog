...
class BaseOrm:
    ...
    def insertbysql(self,*args):
        with SafeDB(self.__source__) as _db:
            return apply(_db.execute_lastrowid,args)
    ...
    def paginate(self,page=1,per_page=20):
        ...
        return {'total': _count, 'data': _data, 'page':page, 'per_page': per_page}
    ...
    def update (self,params={},**kwargs):
        ...
        _lst = _dct.keys()
        self._sql += """update %s set """ % self.__tablename__ + ','.join(["`%s`=%%s" % i for i in _lst]) + " "
    ...
    def replace(self,params={},**kwargs):
        self._type = 'r'; self._sql = ''; self._para = []; self._sql_kv = {}
        if self.__primary__ == None: raise Exception("主键异常")
        _dct = dict(params,**kwargs)
        if self.__primary__ not in [i for i in _dct.keys()]: raise Exception("参数不存在主键")
        if self.select_by_pk(_dct[self.__primary__]).get():
            _update_dct = dict([i for i in _dct.iteritems() if i[0]<>self.__primary__])
            return self.update(_update_dct).where(params={self.__primary__:_dct[self.__primary__]})
        else:
    ...
    def where (self,params={},**kwargs):
        ...
        if len(_dct)==0: pass
            ...
            _lst = _dct.keys()
            self._sql += " where " + ' and '.join(["`%s` = %%s" % i for i in _lst]) + " "
            for i in _lst:
    ...
    def execute_lastrowid (self):
        ...
        if self._db == None:
            with SafeDB(self.__source__) as db:
                return apply(db.execute_lastrowid,tuple([self._sql] + self._para))
        else:
            return apply(self._db.execute_lastrowid,tuple([self._sql] + self._para))
